...
from mini_llm.tokenizer import Tokenizer, train_bpe
from mini_llm.model import Linear, Embedding, RMSNorm, SiLU, SwiGLU, RoPE, RoPE_Qwen, softmax, scaled_dot_product_attention, MultiHeadSelfAttention, TransformerBlock, TransformerLM
from mini_llm.train import cross_entropy_loss, AdamW, get_lr_cosine_schedule, get_batch, clip_gradients, load_checkpoint, save_checkpoint
def run_linear(
    d_in: int,
    d_out: int,
...
def run_transformer_lm(
    ...
    # 构造 state_dict
    state_dict = {}
    state_dict['embd.embeddings'] = weights['token_embeddings.weight']
    state_dict['ln.gain'] = weights['ln_final.weight']
    state_dict['output_embd.weight'] = weights['lm_head.weight']
    # 加载每层的权重
    for i in range(num_layers):
        prefix = f'layers.{i}.'
        state_dict[f'layers.{i}.attn.Wq.weight'] = weights[f'{prefix}attn.q_proj.weight']
        state_dict[f'layers.{i}.attn.Wk.weight'] = weights[f'{prefix}attn.k_proj.weight']
        state_dict[f'layers.{i}.attn.Wv.weight'] = weights[f'{prefix}attn.v_proj.weight']
        state_dict[f'layers.{i}.attn.Wo.weight'] = weights[f'{prefix}attn.output_proj.weight']
        state_dict[f'layers.{i}.ln1.gain'] = weights[f'{prefix}ln1.weight']
        state_dict[f'layers.{i}.ffn.w1.weight'] = weights[f'{prefix}ffn.w1.weight']
        state_dict[f'layers.{i}.ffn.w2.weight'] = weights[f'{prefix}ffn.w2.weight']
        state_dict[f'layers.{i}.ffn.w3.weight'] = weights[f'{prefix}ffn.w3.weight']
        state_dict[f'layers.{i}.ln2.gain'] = weights[f'{prefix}ln2.weight']
    # 加载权重
    # import pdb; pdb.set_trace()
